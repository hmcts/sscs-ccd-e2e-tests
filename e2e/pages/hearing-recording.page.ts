...
export class HearingRecordingPage extends AnyPage {
  ...
  async selectHearing() {
    await element(by.id('selectHearingDetails'))
      .element(by.xpath('//*[@id='selectHearingDetails']/option[2]'))
      .click();
  }
  ...
  async refuseAppellantHearingRecording(permissionType: string) {
    await anyCcdPage.chooseOptionContainingText(
      '#processHearingRecordingRequest_appellant',
      permissionType
    );
  }
  ...
  async grantRequestDwpHearingRecording(permissionType: string) {
    await anyCcdPage.chooseOptionContainingText(
      '#processHearingRecordingRequest_dwp',
      permissionType
    );
  }
}